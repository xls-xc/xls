...
/**
    ...
    public String uploadFile(MultipartFile file) throws ShopException {
        ...
        try {
            ...
            logger.info("------ aliyun oss uploadImage start ------");
            logger.info(result.getETag());
            logger.info(result.getCallbackResponseBody());
            logger.info(result.getRequestId());
            logger.info(key);
            logger.info("------ aliyun oss uploadImage end ------");
            return key;
        } catch (Exception ex) {
            ex.printStackTrace();
...
/**
    ...
    public void createQRCode(String text, String path, String name) {
        ...
        try {
            ...
            String key = "qr/" + path + "/" + name + ".gif";

            PutObjectResult result = ossClient.putObject(bucketName, key, is);

            logger.info("------ aliyun oss qr start ------");
            logger.info(result.getETag());
            logger.info(result.getCallbackResponseBody());
            logger.info(result.getRequestId());
            logger.info(key);
            logger.info("------ aliyun oss qr end ------");
        } catch (Exception e) {
            e.printStackTrace();
...
/**
    ...
    public String createShareQRCard(String imageUrl, String text, String price , String url) throws ShopException {
        ...
        try {
            ...
            String md5ImageName = ShopHelper.getMd5(url);
            QrCard qrCardHelper = new QrCard(640, 960);
            ...
            BufferedImage bufferedImage = qrCardHelper.createQrcardForOss();
            ByteArrayOutputStream os = new ByteArrayOutputStream();
            ImageIO.write(bufferedImage, "png", os);
            InputStream is = new ByteArrayInputStream(os.toByteArray());

            String key = "qr/shareqrcard/" + md5ImageName + ".png";

            PutObjectResult result = ossClient.putObject(bucketName, key, is);

            logger.info("------ aliyun oss qrcard start ------");
            logger.info(result.getETag());
            logger.info(result.getCallbackResponseBody());
            logger.info(result.getRequestId());
            logger.info(key);
            logger.info("------ aliyun oss qrcart end ------");
...
/**
    ...
    public String createWeixinQRCodeForQrScene(String url, int codeId) throws ShopException{
        try {
            ...
            QrCard qrCardHelper = new QrCard(522, 566);
            ...
            String key = "qr/weixin/qrscene/" + imageName + ".png";
            ...
            logger.info("生成微信临时二维码:OssUploadHelper->createWeixinQRCodeForQrScene->异常为" + e.getMessage());
